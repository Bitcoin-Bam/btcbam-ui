<template>
  <div class="columns is-multiline transaction-item">
        ...
        <div v-for="input in inputs" class="is-clearfix">
          ...
          <div class="script">
            ...
            <div v-if="input.scriptSig.asm">
              ...
              <code class="value"><!--
                -->{{ input.scriptSig.asm | btcbam-script }}<!--
              --></code>
            </div>
          ...
          <div class="script">
            ...
            <div v-if="output.scriptPubKey.asm">
              ...
              <code class="value" :class="{'script-code': contractInfo[index]}"
                ...
                -->{{ output.scriptPubKey.asm | btcbam-script }}<!--
              --></code>
            </div>
    ...
    <template v-for="({address, name, symbol, decimals, from, to, value}, index) in brc20TokenTransfers">
      <div class="column is-full flex-full"></div>
      <AttributeInjector
        class="column collapse token-transfer-list"
        :class="{
          'first-item': index === 0,
          'last-item': index === brc20TokenTransfers.length - 1
        }">
        <div class="is-clearfix">
    ...
    <template v-for="({address, name, symbol, from, to, tokenId}, index) in brc721TokenTransfers">
      <div class="column is-full flex-full"></div>
      <AttributeInjector
        class="column collapse token-transfer-list"
        :class="{
          'first-item': index === 0,
          'last-item': index === brc721TokenTransfers.length - 1
        }">
        <div class="is-clearfix">
          <AddressLink v-if="from" :address="from" class="is-pulled-left" :highlight="highlightAddress" />
          <template v-else>{{ $t('contract.token.mint_tokens') }}</template>
    ...
    <div class="column is-full has-text-right collapse-bottom" v-if="fees !== '0'">
      <template v-if="fees > 0">
        {{ $t('transaction.fee') }} <span class="amount fee">{{ fees | btcbam }} BTCBAM</span>
      </template>
      <template v-else-if="fees < 0">
        {{ $t('transaction.reward') }} <span class="amount fee">{{ -fees | btcbam }} BTCBAM</span>
      </template>
    </div>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      brc20TokenTransfers() {
        return this.transaction.brc20TokenTransfers
      },
      brc721TokenTransfers() {
        return this.transaction.brc721TokenTransfers
      },
      contractInfo() {
